...
const CHANNEL_STYLES: Record<string, string> = {
  ...
  'Paid Social': 'bg-violet-50 text-violet-700 border-violet-200',
  Display: 'bg-pink-50 text-pink-700 border-pink-200',
  ...
  Affiliate: 'bg-teal-50 text-teal-700 border-teal-200',
...
function ChannelBadge({ channel }: { channel: string }) {
  ...
  const style = CHANNEL_STYLES[channel] || 'bg-gray-100 text-gray-600 border-gray-200';
  return (
    <span
...
function TrackingInfo({ lead }: { lead: Lead }) {
  const items: { label: string; value: string; isLink?: boolean }[] = [
    ...
    { label: 'Meta Click ID', value: lead.fbclid },
    { label: 'Referrer', value: lead.sourceReferrer, isLink: true },
    { label: 'Entry Landing', value: lead.entryLanding, isLink: true },
  ].filter((i) => i.value);

  return (
    <div className="mt-4 pt-4 border-t border-gray-200">
      <div className="flex items-center gap-2 mb-3">
        <p className="text-xs font-semibold text-gray-500 uppercase">Tracking Info</p>
        <ChannelBadge channel={lead.channel} />
      </div>
      {items.length === 0 ? (
        <p className="text-xs text-gray-400">
          No campaign or referrer data captured for this visitor.
        </p>
      ) : (
